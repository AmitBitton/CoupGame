...
public:
    ...
    size_t get_next_active_index_after(Player* p) const; // Finds next active player after the given one

    // --- Tax block management ---
    bool is_waiting_tax_block() const; // Is a tax block in progress
    void set_waiting_tax_block(bool val);  // Sets the tax block flag
    Player* get_tax_target() const; // Returns the player being taxed
    void set_tax_target(Player* p); // Sets the player being taxed
    Player* get_tax_source() const; // Returns the player who initiated tax
    void set_tax_source(Player* p); // Sets the player who initiated tax
    ...
    bool get_waiting_for_coup_block() const { return waiting_for_coup_block; } // Returns if coup block is in progress
    ...
    bool is_waiting_bribe_block() const { // Is a bribe block in progress
        return waiting_for_bribe_block;
    }

    Player* get_bribing_player() const { return bribing_player; } // Returns the player who bribed
    bool get_waiting_for_bribe_block() const { return waiting_for_bribe_block; } // Returns if bribe block is in progress
